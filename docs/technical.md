...
RERANK_MIN_SCORE strategy (why it is -2.0 in code):
- The code uses a reranker score floor `RERANK_MIN_SCORE = -2.0` (see `AI_module/core/reranking.py`) as a conservative filter to drop only very low-scoring chunks.
- The manual threshold table above shows that there is no clean separation between relevant and irrelevant chunks (the score distributions overlap). Because of that, the threshold is not “reliable” as a classifier.
- Even though the table suggests that a higher threshold around `T = 0` reduces total FP+FN, it also increases FN (drops more potentially relevant chunks). In the RAG setting, higher FN often harms answer quality more than higher FP (LLM gets less evidence and may fail to answer even when the document contains the information).
- Therefore, `-2.0` was chosen as a practical trade-off: keep more candidate chunks (lower FN) while still removing the worst-scoring ones, and rely on additional heuristics (e.g. keeping only chunks close to the top score via `RERANK_MAX_SCORE_GAP`) and the LLM prompt rules to avoid using irrelevant context.
